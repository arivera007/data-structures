...
def sort_by_cohort(filename):
    ...
    all_students = [sorted(winter_16), sorted(spring_16), sorted(summer_16), sorted(fall_15)]
...
def hogwarts_by_house(filename):
    ...
    all_students = [sorted(gryffindor), sorted(hufflepuff), sorted(slytherin), sorted(dumbledores_army), sorted(ravenclaw), sorted(ghosts), sorted(instructors)]
    cohort_data.close()
...
def all_students_tuple_list(filename):
    ...
    for line in cohort_data:
        ...
        if house != "":
            student = ([words[0] + ' ' + words[1], words[2], words[3], words[4].rstrip()])
